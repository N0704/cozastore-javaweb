					...
					<div class="card">
						<div class="card-body">
							<div class="card-header">
								<h4 class="card-title2">Quản lý sản phẩm</h4>
							...
							<table class="table table-borderless datatable">
								<thead>
									<tr>
										...
										<th class="col-6 text-center">Tên sản phẩm</th>
									...
									<c:forEach items="${listP}" var="p">
										<tr>
											<th class="text-center" scope="row"><img
												class="card-img" src="${p.img}" alt="${p.pname}"></th>
											<td><a href="detail?pid=${p.pid}" class="title-link">${p.pname}</a></td>
											<td class="text-center"><fmt:formatNumber
													value="${p.price}" pattern="#,##0" />₫</td>
											<td class="text-center"><a
												href="loadProduct?pid=${p.pid}"
												class="btn btn-outline-dark btn-sm" title="Edit"> <i
													class="bi bi-pencil-square"></i>
											</a> <a href="#" class="btn btn-outline-danger btn-sm delete-btn"
												data-pid="${p.pid}" title="Delete"> <i
													class="bi bi-trash"></i>
											</a></td>
										</tr>
									</c:forEach>
				...
				<form action="add-product" method="post"
						...
						<div class="container-fluid">
							<div class="row">
								...
								<div class="col-md-7 mb-4">
									<div class="mb-4">
										<label for="pname" class="form-label text-dark fw-bold">Tên
											sản phẩm</label> <input name="pname" type="text"
											class="form-control border-dark" id="pname"
											 placeholder="Nhập tên sản phẩm"
											required>
									</div>

									<div class="mb-4">
										<label for="price" class="form-label text-dark fw-bold">Giá
											tiền</label> <input name="price" type="text"
											class="form-control border-dark" id="price"
											placeholder="Nhập giá tiền"
											required>
									</div>

									<div class="mb-4">
										<label for="pdesc" class="form-label text-dark fw-bold">Thông
											tin sản phẩm</label>
										<textarea name="pdesc" class="form-control border-dark"
											id="pdesc" rows="5" placeholder="Nhập thông tin sản phẩm..."></textarea>
									</div>

									<div class="mb-4">
										<label for="editCdesc" class="form-label text-dark fw-bold">Hướng
											dẫn bảo quản</label>
										<textarea name="pci" class="form-control border-dark" id="pci"
											rows="5" placeholder="Nhập hướng dẫn bảo quản..."></textarea>
									</div>
									...
									<div class="form-group mb-4">
										<label for="collection_id"
											class="form-label text-dark fw-bold">Thuộc bộ sưu
											tập:</label> <select class="form-select border-dark"
											name="collection_id" id="collection_id">
											<option value="0">-- Không có --</option>
											<c:forEach items="${listCC}" var="o">
												<option value="${o.collection_id}">${o.name}</option>
											</c:forEach>
										</select>
									</div>
								...
								<div class="col-md-5 mb-4">
									<label class="form-label text-dark fw-bold mb-2">Ảnh
										sản phẩm <span class="text-danger">*</span>
									</label>
									<div
										class="upload-box position-relative d-flex flex-column justify-content-center align-items-center"
										id="imageUploadBox">
										<input type="file" class="d-none" name="cimage" id="cimage"
											accept="image/*" required>

										<!-- Prompt -->
										<div id="imagePrompt" class="text-center">
											<i class="bi bi-cloud-upload fs-1 text-muted"></i>
											<p class="mb-1 fw-semibold">Kéo thả hình ảnh vào đây hoặc
												nhấn để chọn</p>
									...
									<small id="imageError" class="text-danger d-none">Vui
										lòng chọn ảnh hợp lệ</small>
										
									<div class="mt-4">
										<label for="stock" class="form-label text-dark fw-bold">Số lượng hàng:</label> 
										<input name="stock" type="text"
											class="form-control border-dark" id="stock" value="0"
											required>
									</div>
										
									<div class="mt-4">
										<label for="discount" class="form-label text-dark fw-bold">Giảm giá (%)</label> 
										<input name="discount" type="text"
											class="form-control border-dark" id="discount" value="0"
											required>
									</div>
								</div>
							</div>
					...
					<div class="modal-footer border-top-0">
						<button type="button"
							...
							onclick="window.location.href='category-manager'">Hủy bỏ</button>
						<button type="submit" class="btn btn-dark rounded-pill px-4"
							id="submitBtn">Lưu lại</button>
					</div>
				</form>
			</div>
		</div>
				...
				<form action="edit-product" method="post"
					...
					<div class="modal-header bg-dark text-white">
						...
						<button type="button" class="btn-close btn-close-white"
							...
							onclick="window.location.href='product-manager'"></button>
					</div>
					<div class="modal-body py-4">
						<div class="container-fluid">
							<div class="row">
								...
								<div class="col-md-7 mb-4">
									<div class="mb-4">
										<label for="pname" class="form-label text-dark fw-bold">Tên
											sản phẩm</label> <input name="pname" type="text"
											class="form-control border-dark" id="pname"
											value="${detail.pname}" placeholder="Nhập tên sản phẩm"
											required>
									</div>
									...
									<div class="mb-4">
										<label for="price" class="form-label text-dark fw-bold">Giá
											tiền</label> <input name="price" type="text"
											class="form-control border-dark" id="price"
											value="${detail.price}" placeholder="Nhập giá tiền" required>
									</div>
									...
									<div class="mb-4">
										<label for="pdesc" class="form-label text-dark fw-bold">Thông
											tin sản phẩm</label>
										<textarea name="pdesc" class="form-control border-dark"
											id="pdesc" rows="5" placeholder="Nhập thông tin sản phẩm...">${detail.pdesc}</textarea>
									</div>

									<div class="mb-4">
										<label for="pci" class="form-label text-dark fw-bold">Hướng
											dẫn bảo quản</label>
										<textarea name="pci" class="form-control border-dark" id="pci"
											rows="5" placeholder="Nhập hướng dẫn bảo quản...">${detail.pci}</textarea>
									</div>
									...
									<div class="form-group mb-4">
										<label for="collection_id"
											...
											<option value="0">-- Không có --</option>
											<c:forEach items="${listCC}" var="o">
												<option value="${o.collection_id}"
													<c:if test="${detail.collection_id == o.collection_id}">selected</c:if>>
													${o.name}</option>
											</c:forEach>
										</select>
									</div>
								...
								<div class="col-md-5 mb-4">
									...
									<div
										class="upload-box position-relative d-flex flex-column justify-content-center align-items-center"
										id="editImageUploadBox">
										<input type="file" class="d-none" name="cimage"
											id="editCimage" accept="image/*">

										<!-- Hiển thị ảnh hiện tại -->
										<c:if test="${not empty detail.img}">
											<div id="currentImage" class="text-center">
												<img src="${detail.img}" alt="Current Image"
													class="img-fluid mt-2 rounded shadow-sm"
													style="max-height: 220px;">
												<div class="mt-2">
													<button type="button"
														class="btn btn-outline-dark btn-sm rounded-pill"
														onclick="showUploadEdit()">
														<i class="bi bi-arrow-repeat me-1"></i> Thay ảnh
													</button>
												</div>
										...
										<div id="editImagePrompt"
											class="text-center ${not empty detail.img ? 'd-none' : ''}">
											<i class="bi bi-cloud-upload fs-1 text-muted"></i>
											<p class="mb-1 fw-semibold">Kéo thả hình ảnh vào đây hoặc
												nhấn để chọn</p>
											<small class="text-muted">JPG, PNG, tối đa 5MB</small>
										...
										<div id="editImageConfirmed" class="d-none text-center">
											...
											<div class="mt-2">
												<button type="button"
													class="btn btn-outline-dark btn-sm rounded-pill"
													onclick="changeEditImage()">
													<i class="bi bi-arrow-repeat me-1"></i> Thay ảnh
												</button>
											</div>
										</div>
									</div>
									<small id="editImageError" class="text-danger d-none">Vui
										lòng chọn ảnh hợp lệ</small>
									
									<div class="mt-4">
										<label for="stock" class="form-label text-dark fw-bold">Số lượng hàng:</label> 
										<input name="stock" type="text"
											class="form-control border-dark" id="stock" value="${detail.stock}"
											required>
									</div>	
									<div class="mt-4">
										<label for="discount" class="form-label text-dark fw-bold">Giảm giá (%)</label> 
										<input name="discount" type="text"
											class="form-control border-dark" id="discount" value="${detail.discount}"
											required>
									</div>
								</div>
							</div>
					...
					<div class="modal-footer border-top-0">
						<button type="button"
							class="btn btn-outline-secondary rounded-pill px-4"
							data-bs-dismiss="modal"
							onclick="window.location.href='product-manager'">Hủy bỏ</button>
						<button type="submit" class="btn btn-dark rounded-pill px-4"
							id="editSubmitBtn">Lưu lại</button>
					</div>
				</form>
			</div>
		</div>
	...
	document.addEventListener("DOMContentLoaded", function() {
	    ...
	    deleteButtons.forEach(button => {
	        button.addEventListener("click", function(event) {
	            ...
	            confirmDeleteBtn.href = "delete-product?pid=" + pid;
